...
results <- mapply(function(p_curr, q_curr) {
  c(
    Laspeyres = laspeyres_index(p1 = p_curr, p0 = p_base, q0 = q_base),
    Paasche   = paasche_index(p1 = p_curr, p0 = p_base, q1 = q_curr),
...
# 4. Display result
df_indexes[, c("Period", "Laspeyres", "Paasche", "Jevons")]
...
df_long <- pivot_longer(df_indexes, 
                        cols = c("Laspeyres", "Paasche", "Jevons"), 
                        names_to = "Index_Type", 
                        values_to = "Value")
...
ggplot(df_long, aes(x = Period, y = Value, color = Index_Type, group = Index_Type)) +
  ...
  labs(title = "Comparison of Price Indexes (Base = t1)",
       subtitle = "Calculated using gpindex price6/quantity6 data",
       x = "Time Period",
       y = "Index Value",
       color = "Formula")
``` 