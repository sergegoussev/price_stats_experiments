...
-   Step 3: If an item is missing – impute its price once by multiplying the last observed price by the Jevons of the category movement (i.e. 'class mean imputation' method). This is needed as a strict matched-item method will exclude temporary observed items from the computation.
